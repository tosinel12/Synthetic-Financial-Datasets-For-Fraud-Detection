...
#print('training time:' time()-b)
#c=time()
y_pred = dt.predict(X_test)
acc = accuracy_score(y_test, y_pred)
#pint('predicting time:' time()-a)
...
# acc equals to 0.9998
probabilities = dt.fit(X_train, y_train).predict_proba(X_test)
print('AUPRC = {}'.format(
    average_precision_score(y_test, probabilities[:, 1])))
#AUPRC = 0.972179762495006
...
# Cross validation confisuion matrix wiht cv = 5
from sklearn.model_selection import cross_val_predict
y_pred_all = cross_val_predict(clf, X, y, cv=5)
